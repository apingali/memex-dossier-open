...
===========================================
This library ships with a command line program ``memex_dossier.etl`` which
provides a rudimentary pipeline for transforming data from your database
to feature collections managed by :mod:`memex_dossier.store`.

(Currently, ``memex_dossier.etl`` is hard-coded to support a specific HBase
...
from memex_dossier.fc import FeatureCollection, StringCounter
import memex_dossier.models.features as features
...
class to_memex_dossier_store(Configured):
    ...
    '''
    config_name = 'to_memex_dossier_store'
    default_config = {
        'tfidf_path': None
    }
    def __init__(self, *args, **kwargs):
        super(to_memex_dossier_store, self).__init__(*args, **kwargs)
        kvl = kvlayer.client()
        feature_indexes = None
        try:
            conf = yakonfig.get_global_config('memex_dossier.store')
    ...
    def process(self, t_path, name_info, i_str):
        ...
        `t_path` into a :class:`~memex_dossier.fc.FeatureCollection` and saves it in
        a :class:`~memex_dossiery.store.Store` configured with the global `kvlayer`
        config.
